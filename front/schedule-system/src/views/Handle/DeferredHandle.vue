<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop="information" label="申请内容" min-width="200" show-overflow-tooltip />
...
import { 
  getExamDelayApplyList, 
  batchApproveExamDelayApply, 
  batchRejectExamDelayApply,
  approveExamDelayApply, 
  rejectExamDelayApply
} from '@/api/deferredHandle/examDelayHandle'
import { ExamDelayHandleParams, ExamDelayHandleSearchParams, ExamDelayHandleItem } from '@/api/deferredHandle/types'
import { ElMessage, ElMessageBox } from 'element-plus'

const tableData = ref<ExamDelayHandleItem[]>([])
const loading = ref(false)
const selectedRows = ref<ExamDelayHandleItem[]>([])
const dialogVisible = ref(false)
const reviewForm = ref({
  reviewComments: '',
  action: '',
  currentRow: null as ExamDelayHandleItem | null
})

// 搜索表单
const searchForm = ref<ExamDelayHandleSearchParams>({
  applyId: undefined,
  studentId: undefined,
  courseId: undefined,
  state: undefined
...
// 获取缓考申请列表
const fetchExamDelayApplyList = async () => {
  try {
    loading.value = true
    const response = await getExamDelayApplyList(searchForm.value)
...
// 处理表格选择变化
const handleSelectionChange = (selection: ExamDelayHandleItem[]) => {
...
const handleBatchApprove = async () => {
  ...
  try {
    ...
    const ids = selectedRows.value.map((row: ExamDelayHandleItem) => row.applyId)
    const response = await batchApproveExamDelayApply(ids)
    
    if (response.data.IsSuccess) {
      ElMessage.success('批量同意成功')
      await fetchExamDelayApplyList()
      selectedRows.value = []
    } else {
...
const handleBatchReject = async () => {
  ...
  try {
    ...
    const ids = selectedRows.value.map((row: ExamDelayHandleItem) => row.applyId)
    const response = await batchRejectExamDelayApply(ids)
...
// 同意申请
const handleApprove = (row: ExamDelayHandleItem) => {
  reviewForm.value = {
    reviewComments: '',
    action: 'approve',
    currentRow: row
...
// 拒绝申请
const handleReject = (row: ExamDelayHandleItem) => {
  reviewForm.value = {
    reviewComments: '',
    action: 'reject',
    currentRow: row
...
const confirmReview = async () => {
  ...
  try {
    ...
    let response
    if (action === 'approve') {
      response = await approveExamDelayApply(applyId, reviewComments, reviewerId)
    } else {
      response = await rejectExamDelayApply(applyId, reviewComments, reviewerId)
    }
