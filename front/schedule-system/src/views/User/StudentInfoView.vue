  ...
  <div class="student-info-container">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <el-table-column prop="TypeDetail" label="学历" width="100" align="center" />
        <el-table-column label="状态" width="100" align="center">
          <template #default="scope">
            <el-tag :type="scope.row.IsGraduated ? 'success' : 'primary'">
              {{ scope.row.IsGraduated ? '已毕业' : '在读' }}
            </el-tag>
          </template>
        </el-table-column>
...
import { getStudentInfoList, getMajorList } from '@/api/studentInfo/studentInfo';
import type { MajorInfo, StudentInfoResult } from '@/api/studentInfo/types';
import { ElMessage } from 'element-plus';

// 状态管理
const loading = ref(false);
const tableData = ref<StudentInfoResult[]>([]);
...
const loadStudentList = async () => {
  try {
    ...
    if (res.data.IsSuccess) {
      ...
      tableData.value = [];
      pagination.total = 0;
      ElMessage.error(res.data.Message || '获取学生信息失败');
    }
  } catch (err) {
...
// 监听搜索条件变化（可选）
watch([() => searchForm.name, () => searchForm.majorName, () => searchForm.gender], 
  (newVals, oldVals) => {
    // 可以添加防抖处理
  }
);
...
.student-info-container {
  padding: 20px;
  padding-bottom: 80px; /* 增加底部空白，数值可根据需要调整 */
